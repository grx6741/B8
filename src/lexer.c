...
{
	return ( lexer_t ) { .fd = fd, .row = 1, .col = 1, .file_name = file_name };
}
...
token_t LexerNextToken( lexer_t* L )
{
	...
	if ( c == '/' ) {
		...
		if ( strcmp( id_name, "loop" ) == 0 )
			return NEW_TOKEN( kTokenLoop, .row = L->row, .col = start_col );
		if ( strcmp( id_name, "break" ) == 0 )
			return NEW_TOKEN( kTokenBreak, .row = L->row, .col = start_col );
